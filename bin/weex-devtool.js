...
program.option('-v, --version', 'display version').
  ...
  option('--loglevel [loglevel]',
    'set log level silent|error|warn|info|log|debug', 'error').
  option('--remotedebugport [remotedebugport]', 'set the remote debug port',
...
if (program.loglevel) {
  program.loglevel = program.loglevel.toLowercase &&
    program.loglevel.toLowercase()
...
process.on('unhandledRejection', (reason, p) => {
  ...
  if (/simctl error/.test(reason)) {
    logger.warn(
      `The simulator debug need Xcode environment, you can run \`simctl --version\` to check if you have the correct environment.`)
  }
